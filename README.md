...
- `원가_업데이트_필요_[타임스탬프].txt` - GPT 자동 생성 SKU의 원가 업데이트 안내 (쿠팡 2P만 해당)
  - 신규 생성된 SKU 목록 (ID, 상품명, 분류, 현재 원가)
  - 담당자에게 전달용 (복사-붙여넣기 가능)
...
**주요 기능**:
- 개수변경 자동생성 감지 (예: 500ml 1병 → 500ml 2병, 10개입 → 20개입)
- 환불/재판매 상품 감지
- 신규 상품 자동 등록 (용량/패키지 변경 포함, 예: 500ml → 1L)
- ID_master 자동 생성 (알파벳+숫자 형식 지원, 예: S060704101):
  - 개수변경 (같은 패키지): `S060704101-1`, `S060704101-2`...
  - 환불재판매: `S060704101-A`, `S060704101-B`...
  - 신규상품 (용량변경 포함): `S060704102`, `S060704103`...
- 원가 자동 계산 (케이스 1):
  - 상품명에서 개수 자동 추출 (예: "2병", "3개입", "5팩")
  - 원가 = 기초 원가 × (신규 개수 / 기초 개수)
  - 예: "500ml 1병" (원가 1000원) → "500ml 2병" (원가 2000원)
- 원가 처리 (케이스 2, 3):
  - 케이스 2 (환불재판매): 기초 원가 × 0.8 (20% 할인)
  - 케이스 3 (신규상품): 0으로 설정 (손익계산 오류 방지)
  - 추후 SKU_master 테이블에서 실제 원가 수동 업데이트 가능
- 원가 업데이트 보고서 자동 생성:
  - 신규 생성 SKU가 있을 경우 `원가_업데이트_필요_[타임스탬프].txt` 파일 자동 생성
  - 화면에 보고서 출력 + 파일 저장
  - 담당자에게 원가 업데이트 요청 시 활용 가능
...
5. **원가 정보**: GPT 자동 생성 SKU는 원가가 0으로 설정됩니다. 담당자에게 원가 업데이트를 요청하여 SKU_master 테이블에 실제 원가를 반영해야 정확한 손익분석이 가능합니다.
