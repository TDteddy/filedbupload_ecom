...
1. Quantity-modified auto-generated items
...
class CoupangProductAnalyzer:
    ...
    def analyze_product(self, unmatched_product, existing_skus):
        ...
        Returns:
            Dict with:
                - case_type: int (1: 수량변경, 2: 환불재판매, 3: 신규상품)
...
**케이스 1 (수량변경 자동생성)**: 쿠팡이 기존 상품의 수량만 변경하여 자동으로 새 옵션을 생성한 경우
- 특징: 제품명이 거의 동일하지만 수량, 개수, 용량 표기만 다름 (예: "10개입" → "20개입", "500ml" → "1L")
- 같은 브랜드, 같은 카테고리
- 기존 제품의 배수 관계
...
예시:
- 수량변경: {"case_type": 1, "base_master_id": "42", "confidence": 0.95, "reasoning": "기존 '500ml' 제품과 브랜드/카테고리 동일하나 '1L'로 용량만 2배 증가"}
- 환불재판매: {"case_type": 2, "base_master_id": "42", "confidence": 0.9, "reasoning": "옵션명에 '리퍼' 키워드 포함, 기존 제품과 동일"}
- 신규상품: {"case_type": 3, "base_master_id": null, "confidence": 0.85, "reasoning": "기존 DB에 해당 브랜드나 유사 제품 없음"}
"""
